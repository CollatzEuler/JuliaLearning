...
function GA(fitness_function, crossover, mutator, parents_per_child, mutation_rate, population_size, time_limit, num_cities, distances)
    ...
    while time() - time_start < time_limit
        ...
        for i in 1:population_size
            ...
            @assert length(unique(child)) == num_cities
            @assert sort(child) == collect(1:num_cities)
            # @assert length(child) == num_cities && child == unique(child), "Child length mismatch: expected $num_cities, got $(length(child))"
        ...
        population = sort(population, by=x -> fitness_function(x, distances))  # Sort by fitness
        population = population[1:population_size]  # Keep the best individuals
...
function crossover_partition(parents, num_cities)
    ...
    for i in 1:num_parents
        ...
        for j in seg_start:seg_end
            val = parent[j]
            if !(val in assigned) && child[j] == -1  # <- prevent overwriting
                child[j] = val
                push!(assigned, val)
            end
        end
    ...
    unused = collect(setdiff(1:num_cities, assigned))
    shuffle!(unused)
    remaining_slots = findall(x -> x == -1, child)

    @assert length(unused) == length(remaining_slots) "Mismatch: $(length(unused)) unused but $(length(remaining_slots)) slots"
...
start = time()
best_state, best_cost, iters = GA(fitness, crossover_partition, mutate_swap, parents_per_child, mutation_rate, population_size, time_limit, num_cities, distances)
elapsed = time() - start
